...
大白话：react的高阶组件吧，用来在异步加载组件的时候，显示加载页面
...
+ Suspense的fallback是会等待所有子组件都加载完成，才会显示。

nextJs中如何处理 Suspense：
...
+ **Stream Server Rendering （流式渲染）**：注意和SSR不一样，SSR（Server-Side Render） 是服务端渲染。
  + Stream Server Render 解决了 SSR渲染的第一个问题，就是数据获取必须在组件渲染之前。可以先渲染Fallback UI ，等数据返回再渲染具体的组件内容。
  + 具体过程： 
...
所以定义loading.js。默认会被`<Suspense>`包裹。

所以一些路由切换的时候，可以使用loading.js实现优化。
